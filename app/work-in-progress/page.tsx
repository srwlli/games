...
export default function WorkInProgressPage() {
  // Filter games by "Work In Progress" category
  const wipGames = Object.values(GAMES_REGISTRY).filter((game) => game.category === "Work In Progress")
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-6 pb-20 font-sans">
      ...
      <header className="max-w-6xl mx-auto mb-12 mt-8">
        ...
        <div>
          <h1 className="text-5xl font-black italic tracking-tighter uppercase leading-none">
            Work In<span className="text-orange-500">Progress</span>
          </h1>
          <p className="text-zinc-500 mt-2 font-medium uppercase tracking-widest text-xs">Games Under Development</p>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {wipGames.map((game) => (
          <Link href={`/work-in-progress/${game.id}`} key={game.id}>
            <motion.div
              whileTap={{ scale: 0.97 }}
              whileHover={{ y: -5 }}
