...
export default function WordGamesPage() {
  // Filter games by "Word Game" category
  const wordGames = Object.values(GAMES_REGISTRY).filter((game) => game.category === "Word Game")
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-6 pb-20 font-sans">
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {wordGames.map((game) => (
          <Link href={`/word-games/${game.id}`} key={game.id}>
            <motion.div
              whileTap={{ scale: 0.97 }}
              whileHover={{ y: -5 }}
