...
export default function MathGamesPage() {
  // Filter games by "Math Game" category
  const mathGames = Object.values(GAMES_REGISTRY).filter((game) => game.category === "Math Game")
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-6 pb-20 font-sans">
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {mathGames.length > 0 ? (
          mathGames.map((game) => (
            <Link href={`/math-games/${game.id}`} key={game.id}>
              <motion.div
                whileTap={{ scale: 0.97 }}
                whileHover={{ y: -5 }}
          ...
          <div className="col-span-full py-20 flex flex-col items-center justify-center border-2 border-dashed border-zinc-800 rounded-3xl text-zinc-500">
            ...
            <p className="text-sm">We're currently building exciting math games for you.</p>
          </div>
        )}
      </main>
    </div>
